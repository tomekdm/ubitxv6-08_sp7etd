...
int change_ssb = 0;//09_sp7etd
int lck = 0; //07_sp7etd - new variable for lock of frequency tuning @ 09_sp7etd
int s_meter_on = 0;//08_sp7etd - by default s-meter is OFF @09_sp7etd
...
void checkButton(){
 ...
 active_delay(100);
  ...
  if (downTime > 0 && downTime < 10 && jog>0) jog--;//02_sp7etd - depending on encoder button down time - jog position right or left
  if (downTime >= 10 && downTime < 40 && jog<4) jog++; //02_sp7etd - depending on encoder button - jog position right or left
...
if (millis() > s_meter_rate && s_meter_on==1) s_meter();//08_sp7etd - calling s-meter in main loop if s-meter button is ON and 300ms passed
  ...
  if (!inTx){
    if (ritOn)
      ...
      if (change_ssb) {setFrequency(frequency); change_ssb = 0;}//09_sp7etd
  }
