...
int jog = 1; //02_sp7etd - new variable for jog type tuning implementation
int touch = 1;//04_sp7etd - new variable for toggle touch ON/OFF - discovered that checking TFT touch is casuing audio noises - possible due to SPI; this variable allows turning TFT touch OFF/ON by 5 second pressing of knob (by default touch ON)
int s_meter_value;//08_sp7etd - for s-meter operation

long s_meter_rate;//08_sp7etd - for s-meter operation
int started;//08_sp7etd - for s-meter operation
/** 
...
unsigned long vfoA=7150000L, vfoB=14200000L, sideTone=600, usbCarrier;//01_sp7etd sidetone to 600
 ...
 */
int jog_export() {return jog;}//03_sp7etd - passing jog value to ubitx.h and then to ubitx_ui.cpp
int touch_export() {return touch;}//04_sp7etd - passing touch value to ubitx.h and then to ubitx_ui.cpp
int s_meter_value_export() {return s_meter_value;}//08_sp7etd - for s-meter operation
...
void setFrequency(unsigned long f){//sp7etd - possible changes to this function in the future (not tested yet - MIND sidetone shift!)
...
*/
  ...
  if (isUSB){
    if (cwMode) { si5351bx_setfreq(2, firstIF  + f + sideTone);  si5351bx_setfreq(1, firstIF - usbCarrier); } //07_sp7etd fixing shift of RX when in CW and USB
    else 
      { si5351bx_setfreq(2, firstIF  + f); si5351bx_setfreq(1, firstIF + usbCarrier);} //07_sp7etd fixing shift of RX when in CW and USB
  }
  else{
    if (cwMode)
...
void startTx(byte txMode){
 ...
 // digitalWrite(TX_RX, 1);//01_sp7etd - smoothing CW TX 
   if (txMode != TX_CW) digitalWrite(TX_RX, 1);//01_sp7etd - smoothing CW TX 
  inTx = 1;
  ...
  if (txMode == TX_CW){
   // digitalWrite(TX_RX, 0);//01_sp7etd - to smooth CW TXing
    ...
    /*//03_sp7etd cancelling shift +/- side tone
    if (isUSB)
      si5351bx_setfreq(2, frequency + sideTone);
    else
      si5351bx_setfreq(2, frequency - sideTone);
*/
si5351bx_setfreq(2, frequency);//03_sp7etd cancelling shift +/- side tone when CW
    //delay(20);//01_sp7etd - removing any CW TX delay for immediate straight key response
...
void checkButton(){
 // int i, t1, t2, knob, new_knob;//01_sp7etd could not find where used - line commented
 ...
 active_delay(100);
  ...
  if (downTime > 0 && downTime < 10 && jog>0) jog--;//02_sptetd - depending on encoder button down time - jog position right or left
  if (downTime >= 10 && downTime < 40 && jog<4) jog++; //02_sptetd - depending on encoder button - jog position right or left
  if (downTime >= 40 && downTime < 100) doCommands();//02_sp7etd
  if (downTime >= 100 && touch==1) touch=0; //04_sp7etd - if button pressed for >5s toggle TFT touch
      else if (downTime >= 100 && touch==0) touch=1;//04_sp7etd - if button pressed for >5s toggle TFT touch
  //doCommands();//02_sp7etd
jogUpdate();//04_sp7etd
...
/*//02_sp7etd - switching to jog tuning
  if (s > 10)
    frequency += 1000l * s;
  else if (s > 5)
    frequency += 500l * s;
...
//02_sp7etd - jog tuning implementation
if (s>0 && jog==0) frequency += 10l;
if (s<0 && jog==0) frequency -= 10l;
if (s>0 && jog==1) frequency += 100l;
if (s<0 && jog==1) frequency -= 100l;
if (s>0 && jog==2) frequency += 1000l;
if (s<0 && jog==2) frequency -= 1000l;
if (s>0 && jog==3) frequency += 10000l;
if (s<0 && jog==3) frequency -= 10000l;
if (s>0 && jog==4) frequency += 100000l;
if (s<0 && jog==4) frequency -= 100000l;
//if (s>0 && jog==5) frequency += 1000000l;
//if (s<0 && jog==5) frequency -= 1000000l;
...
//  pinMode(ANALOG_KEYER, INPUT_PULLUP);
  ...
  pinMode(ANALOG_SPARE, INPUT);//08_sp7etd
}
...
void setup()
{
  ...
  guiUpdate();
  // displayRawText("v6.1", 270, 210, DISPLAY_LIGHTGREY, DISPLAY_NAVY);//04_sp7etd - changing way to display firmware version
}

void s_meter()//08_sp7etd - s-meter function called every 300ms
{
  int temp_s_meter_value;
  started = 0; 
  // s_meter_value = analogRead(ANALOG_SPARE);
  temp_s_meter_value = analogRead(ANALOG_SPARE);

  if (temp_s_meter_value>=0 && temp_s_meter_value<532) s_meter_value=0;
  else if (temp_s_meter_value>=532 && temp_s_meter_value<540) s_meter_value=1;
  else if (temp_s_meter_value>=540 && temp_s_meter_value<560) s_meter_value=2;
  else if (temp_s_meter_value>=560 && temp_s_meter_value<601) s_meter_value=3;
  else if (temp_s_meter_value>=601 && temp_s_meter_value<677) s_meter_value=4;
  else if (temp_s_meter_value>=677 && temp_s_meter_value<826) s_meter_value=5;
  //else if (temp_s_meter_value>=826 && temp_s_meter_value<890) s_meter_value=6;
  else if (temp_s_meter_value>=826 && temp_s_meter_value<=889) s_meter_value=7;
  //else if (temp_s_meter_value=<=892 && temp_s_meter_value=<893) s_meter_value=9;
  else if (temp_s_meter_value>889 && temp_s_meter_value<894) s_meter_value=9;
  else if (temp_s_meter_value==894) s_meter_value=10;
  else if (temp_s_meter_value==895) s_meter_value=20;
  else if (temp_s_meter_value==897) s_meter_value=30;
  else if (temp_s_meter_value==898) s_meter_value=40;
  else if (temp_s_meter_value==899) s_meter_value=50;
  else if (temp_s_meter_value>=900) s_meter_value=60;
...
void loop(){ 
if (started==0) { s_meter_rate = millis() + 300; started = 1;}
  ...
  int lck;//07_sp7etd
  lck=lck_export();//07_sp7etd

int s_meter_on;
s_meter_on=s_meter_on_export();

if (millis() > s_meter_rate && s_meter_on==1) s_meter();//08_sp7etd - calling s-meter in main loop if s-meter button is ON and 300ms passed

  if (!inTx){
    if (ritOn)
      doRIT();
    else 
      if (lck==0) doTuning();//07_sp7etd
      if (touch) checkTouch();//04_sp7etd - do not check touch if touch flag is 0
  }
